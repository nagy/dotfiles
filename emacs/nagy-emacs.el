...
(defun yublin-activate ()
  (interactive)
  (maphash (lambda (key value)
             (define-abbrev text-mode-abbrev-table key value nil :system t))
           yublin--map))

(defun yublin-deactivate ()
  (interactive)
  (maphash (lambda (key _value)
             (define-abbrev text-mode-abbrev-table key nil nil :system t))
           yublin--map))
...
;; (require 'dash)
(eval-and-compile
  (defun defun-shell--convert (entry)
    (pcase entry
      ((pred characterp) (format "%c" entry))
      ((pred stringp) entry)
      ((pred numberp) (number-to-string entry))
      ((pred keywordp)
       (concat
        (if (> (length (symbol-name entry)) 2)
            "--"
          "-")
        (substring (symbol-name entry) 1)))
      ('t "true")
      ((pred null) "false")
      (_ (format "%s" entry)))))

(defmacro defun-shell (name arglist &rest body)
  (declare (indent 2)
           (debug (&define name lambda-list def-body)))
  `(defun ,name ,arglist
     "Autogenerated by `defun-shell'."
     (interactive)
     (with-environment-variables
         (("NO_COLOR" "1"))
       (async-shell-command
        ,(string-join (mapcar #'defun-shell--convert body) " ")
        (with-current-buffer (generate-new-buffer ,(format "€%s" name))
          (read-only-mode 1)
          (current-buffer))))))
...
(defun +toggle-tab-bar-mode-from-frame ()
  ...
  (if (eq tab-bar-show t) (setq tab-bar-show 1))
...
(defun the-shell-here ()
  ...
  (let ((oldbuf (current-buffer)))
    (alet (generate-new-buffer (format "*shell*"))
      (with-current-buffer it
        (let ((process-environment
               (with-current-buffer oldbuf (copy-sequence process-environment))))
          (shell (current-buffer)))
        (setq-local process-environment
                    (with-current-buffer oldbuf (copy-sequence process-environment)))))))
(keymap-global-set "s-x" #'the-shell-here)

(defun ignore-advice (orig-fun &rest args)
  (ignore-errors (apply orig-fun args)))
;; Fix some error on ielm when fontify does not understand functions as matchers
(advice-add 'font-lock-fontify-region :around #'ignore-advice)
...
(defun find-tmp (arg)
  ...
  (let ((default-directory "/"))
    (let* ((num (if (numberp arg) arg (cl-incf tmp-counter)))
      ...
      (find-file dirname))))
(keymap-global-set "H-t" #'find-tmp)
(defun find-TMP ()
...
(cl-defun maplines (function &optional (buffer (current-buffer)))
  "Apply FUNCTION to each line of the buffer and return the results.

When FUNCTION will be called, the point will be at the beginning of the line.

Optionally use BUFFER as the buffer to iterate. Otherwise use current buffer."
  (let (result)
    (with-current-buffer buffer
      (save-excursion
        (goto-char (point-min))
        (while (< (point) (point-max))
          (beginning-of-line)
          (push (save-excursion
                  (funcall function (buffer-substring-no-properties
